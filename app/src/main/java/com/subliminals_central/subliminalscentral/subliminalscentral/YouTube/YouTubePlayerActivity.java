...
public class YouTubePlayerActivity extends YouTubeBaseActivity implements
    ...
    protected void onStop() {
        long videoStoped = new Date().getTime();
        if((videoStoped - videoStarted)>1000*60*2){
            // IF TIME PLAYED > 2 MINUTES
            // CALL OK!
            int current = UserData.getUserPoints();
            current++;

            UserData.setUserPoints(current);
            ...
            String answer = UserAuth.userUpdateScores(current);
